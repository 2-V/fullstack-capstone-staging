  ...
  context "sign-up" do
	...
	context "invalid registration" do
		...
		context "non-unique information" do
			...
			expect(payload["errors"]).to include("email")
			expect(payload["errors"]).to include("full_messages")
			expect(payload["errors"]["full_messages"]).to include(/already been taken/i)
			end
		end
	end
  end
	...
	let!(:user) { login account, :ok }
  	...
  	context "invalid password" do
  		it "rejects credentials" do
  			login_hash = {}
  			login_hash[:name] = user_props[:name]
  			login_hash[:email] = user_props[:email]
  			login_hash[:password] = 12345

  			login login_hash, :unauthorized
  			expect(response).to have_http_status(:unauthorized)
